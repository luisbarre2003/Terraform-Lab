resource "azurerm_resource_group" "LB-Terraform-LAB" {
    name = var.rg_name
...
resource "azurerm_public_ip" "pip-exercise" {
    name     = var.pip_name
    resource_group_name = azurerm_resource_group.LB-Terraform-LAB.name
    location = azurerm_resource_group.LB-Terraform-LAB.location
